#!/usr/bin/env bash
# tests/cases/bash-286-word-boundary-silent.sh — AC-286.
#
# Each of these commands contains a runner name as a SUBSTRING of a longer
# identifier ("tsc" inside "tscheck", "make" inside "maker", "test" inside
# "mytesting-tool"), never as the whole word the regex's `\b` requires, so
# all three must be a silent no-op.
set -u
